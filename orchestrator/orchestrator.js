module.exports = function (RED) {
  ...
  function OrchestratorNode(n) {
    ...
    try {
      node.brokerConn.subscribe(
        ...
        async (topic, payload, packet) => {
          ...
          if (
            Object.prototype.hasOwnProperty.call(nodeAssignment, "deviceId")
          ) {
    ...
    function equalDevices(newDevices) {
      if (
        Object.keys(newDevices).length !== Object.keys(devices).length ||
        Object.keys(newDevices).sort().join(",") !==
          Object.keys(devices).sort().join(",")
      )
        return false;

      for (const id in devices) {
        if (newDevices[id].new && newDevices[id].status !== devices[id].status)
          return false;
        if (
          newDevices[id].new &&
          newDevices[id].failure !== devices[id].failure
        )
          return false;
        if (
          newDevices[id].new &&
          newDevices[id].capabilities.sort().join(",") !==
            devices[id].capabilities.sort().join(",")
        )
          return false;
      }
    ...
    function getBestDevice(availableDevices, flowNode) {
      ...
      for (const id in availableDevices) {
        ...
        if (
          device.status !== 1 ||
          (nodeAssignment[id] && nodeAssignment[id].status !== 1)
        )
          continue;
        ...
        if (
          nodeAssignment[id] &&
          device.memoryErrorNodes &&
          nodeAssignment[id].nodes.length + 1 >= device.memoryErrorNodes
        )
          continue;
        ...
        const predicateIntersection = nodePredicates.filter((tag) =>
          device.capabilities.includes(tag)
        );
        ...
        const prioritiesIntersection = nodePriorities.filter((tag) =>
          device.capabilities.includes(tag)
        );
        const prioritiesIndex =
          prioritiesIntersection.length === 0
            ? 0
            : prioritiesIntersection.length / nodePriorities.length;

        // Nodes with less nodes assigned, more priorities complied and
        // more specific intersections have a better match index
        const nrNodes = nodeAssignment[id]
          ? nodeAssignment[id].nodes.length
          : 0;
        const matchIndex =
          prioritiesIndex * 0.5 +
          (1 / (nrNodes + 1)) * 0.4 +
    ...
    async function distributeFlow(nodes, changes) {
      ...
      Object.entries(nodes).forEach(async ([k, flowNode]) => {
        if (!flowNode.inputTopics) flowNode.inputTopics = "";

        if (!flowNode.outputTopics) flowNode.outputTopics = "";

        if (!flowNode.predicates) flowNode.predicates = [];

        if (!flowNode.priorities) flowNode.priorities = [];

        if (!flowNode.generateCode) flowNode.generateCode = false;

        if (
          flowNode.type === "mqtt-broker" ||
          flowNode.type === "tab" ||
          flowNode.type === "registry"
        ) {
          return;
        }
        ...
        if (
          flowNode.type.includes("subflow") ||
          (!flowNode.noWiresType &&
            flowNode.inputTopics.length === 0 &&
            flowNode.outputTopics.length === 0)
        ) {
        ...
        if (!flowNode.generateCode) {
          if (nodeAssignment["node-red"]) {
            if (
              !nodeAssignment["node-red"].nodes.find(
                (n) => n.id === flowNode.id
              )
            ) {
        ...
        if (deviceId) {
          ...
          if (
            ...
            nodeAssignment[deviceId].nodes.filter((n) => n.id === flowNode.id)
              .length === 0
          ) {
          ...
          if (
            flowNode.predicates.length > 0 &&
            !flowNode.predicates.includes("node-red")
          ) {
            node.error(
              `Predicates in node ${flowNode.id} cannot be satisfied.`
            );
      ...
      for (const id in nodeAssignment) {
        if (
          nodeAssignment[id].status === 0 &&
          nodeAssignment[id].nodes.length > 0
        ) {
          nodeAssignment[id].nodes = [];
        }
      }
    ...
    function sendTelemetryData() {
      setInterval(() => {
        ...
        if (nodeAssignment["node-red"]) {
          payload = {
            nodes:
              nodeAssignment["node-red"].nodes.map((n) => n.id).join() || 0,
