...
module.exports = function (RED) {
    ...
    function IfNode(n) {
        ...
        this.deviceHandler = new DeviceHandler(node, this.brokerConn, generateMicropythonCode(this.property, this.rules))
        ...
        function pythonParameters(values) {
            ...
            else if (values.length === 1) return `, b = ${values[0] === "" ? "\"\"" : values[0]}`;
            else return `, b = ${values[0] === "" ? "\"\"" : values[0]}, c = ${values[1] === "" ? "\"\"" : values[1]}`;
        }
        ...
        function generateMicropythonCode(property, rules) {
            ...
            for (let i = 0; i < rules.length; i++) {
                ...
                const ifRules = `def if_rule_${textId}_${i}(a${pythonParameters(valuesRule)}):\n    ${typeConverter(rules[i].vt)}\n    return ${pythonOperators[rules[i].t]}\n`;

                rulesBlock += i == 0 ? `if_rule_${textId}_${i}(a)` : ` and if_rule_${textId}_${i}(a)`;
                ifBlock += ifRules;
            }
